...
impl Default for IdGeneratorConfig {
    fn default() -> Self {
        let sequence_bits = 18;
        let worker_id_bits: u32 = 2;
        let datacenter_id_bits = 2;
        ...
        Self {
            ...
            datacenter_id_bits,
            timestamp_bits,
            epoch: 1761955200000, // 2025-11-01 00:00:00 UTC
...
impl IDGenerator {
    pub fn new(cfg: IdGeneratorConfig) -> Result<Self> {
        // 验证配置
    ...
    pub fn generate_ids_batch(&self, count: usize) -> Result<Vec<u64>> {
        let seq_bits = self.cfg.sequence_bits as u32;
        let seq_mask: u64 = (1u64 << self.cfg.sequence_bits) - 1;
        let max_seq: u64 = self.cfg.max_sequence as u64;
...
mod tests {
    ...
    fn test_concurrent_id_generation() {
        ...
        // 启动多个线程并发生成ID
        for _ in 0..1002 {
            let gent = Arc::clone(&generator);
            let ids_clone = Arc::clone(&ids);

            let handle = thread::spawn(move || {
                let mut local_ids = Vec::new();
                for _ in 0..1000 {
                    match gent.generate_id() {
    ...
    fn test_default_config_validity() {
        let default_cfg = IdGeneratorConfig::default();
